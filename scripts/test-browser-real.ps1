...
try {
  ...
  $poolFact = & $docker exec $postgresContainer psql -v ON_ERROR_STOP=1 -U llmgateway -d $databaseName -Atc `
    "SELECT (SELECT count(*) FROM resource_pools pool JOIN providers provider ON provider.id = pool.provider_id WHERE pool.slug = 'browser-pool' AND pool.name = 'Browser Pool' AND pool.status = 'active' AND provider.catalog_id = 'siliconflow') || '|' || (SELECT count(*) FROM resource_pool_models binding JOIN resource_pools pool ON pool.id = binding.resource_pool_id JOIN models model ON model.id = binding.model_id WHERE pool.slug = 'browser-pool' AND model.public_name = 'qwen3.5-9b') || '|' || (SELECT count(*) FROM resource_pool_mutations mutation JOIN resource_pools pool ON pool.id = mutation.resource_pool_id WHERE pool.slug = 'browser-pool' AND mutation.action = 'resource_pool.create') || '|' || (SELECT count(*) FROM audit_events audit WHERE audit.action = 'resource_pool.create' AND audit.target_id = (SELECT id::text FROM resource_pools WHERE slug = 'browser-pool') AND audit.request_id IS NOT NULL)"
  ...
  $planFact = & $docker exec $postgresContainer psql -v ON_ERROR_STOP=1 -U llmgateway -d $databaseName -Atc `
    "SELECT (SELECT count(*) FROM service_plans plan JOIN service_plan_versions version ON version.id = plan.current_version_id WHERE plan.slug = 'browser-plan' AND plan.name = 'Browser Plan' AND plan.status = 'active' AND version.version = 1 AND version.token_quota = 50000) || '|' || (SELECT count(*) FROM service_plan_version_routes route JOIN service_plan_versions version ON version.id = route.service_plan_version_id JOIN service_plans plan ON plan.id = version.service_plan_id JOIN models model ON model.id = route.model_id JOIN resource_pools pool ON pool.id = route.resource_pool_id WHERE plan.slug = 'browser-plan' AND model.public_name = 'qwen3.5-9b' AND pool.slug = 'browser-pool') || '|' || (SELECT count(*) FROM service_plan_mutations mutation JOIN service_plans plan ON plan.id = mutation.service_plan_id WHERE plan.slug = 'browser-plan' AND mutation.action = 'service_plan.create') || '|' || (SELECT count(*) FROM audit_events audit WHERE audit.action = 'service_plan.create' AND audit.target_id = (SELECT id::text FROM service_plans WHERE slug = 'browser-plan') AND audit.request_id IS NOT NULL)"
  ...
  Copy-Item -LiteralPath (Join-Path $buildDirectory "getting-started-desktop.png") -Destination (Join-Path $evidenceDirectory "getting-started-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "onboarding-tour-desktop.png") -Destination (Join-Path $evidenceDirectory "onboarding-tour-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "resource-pool-form-desktop.png") -Destination (Join-Path $evidenceDirectory "resource-pool-form-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "upstream-key-form-desktop.png") -Destination (Join-Path $evidenceDirectory "upstream-key-form-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "plan-form-desktop.png") -Destination (Join-Path $evidenceDirectory "plan-form-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "subscription-form-desktop.png") -Destination (Join-Path $evidenceDirectory "subscription-form-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "api-key-form-desktop.png") -Destination (Join-Path $evidenceDirectory "api-key-form-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "administrator-dashboard-desktop.png") -Destination (Join-Path $evidenceDirectory "administrator-dashboard-desktop.png") -Force
  Copy-Item -LiteralPath (Join-Path $buildDirectory "member-dashboard-desktop.png") -Destination (Join-Path $evidenceDirectory "member-dashboard-desktop.png") -Force
