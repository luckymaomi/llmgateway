...
2. 用户与管理员：管理员邀请、审核、停用成员，分配模型权限、额度、RPM、TPM、并发和 Key；成员只管理自己的 Gateway Key，并查看自己的订阅与额度、额度记录和 API 日志。
...
- 上游 API Key 只由管理员维护；Gateway Key 是分配给调用方的下游凭据，两者不能混用。
...
- 内置 Provider 模板安装只把 catalog 已在启动时校验的 HTTPS 端点和文本模型持久化为停用 registry 事实，不解析 DNS、不探测上游，也不接收 secret。凭据就地测试和正式发送继续通过 SSRF-safe transport 校验每次解析与重定向。
...
- 客户端 IP、透明代理 Fake-IP 和出站解析结果不参与用户停用、封禁、审核或会话撤销。受信代理边界下的客户端 IP 只服务明确限流；出站地址只服务 SSRF 策略。
...
管理员与成员共用控制台框架，导航按服务端 capability 投影，服务端权限独立强制。管理员直接操作运营总览、运维监控、Provider、模型、上游 API Key、配置发布、成员、邀请、Gateway Key、订阅与额度、API 日志、额度记录、上游成本和站点设置；成员直接操作仪表盘、自己的订阅与额度、额度记录、Gateway Key、API 日志和账号。成员直接访问管理员页面或 API 仍被拒绝，不能读取上游 secret、采购成本或其他成员事实。

上游 API Key 与 Gateway Key 的真实连接测试分别在所属对象页面完成。上游测试选择已经绑定的模型并走对应 Provider adapter 的最小生成；Gateway Key 测试走统一 request workflow，不建立独立请求实验链。

API 日志读取所有已接受请求，而不是只读取已有 usage 的完成请求。查询由 PostgreSQL 按最长 31 天时间窗、成员、Gateway Key、模型、资源域、状态和 Request ID 分页；状态覆盖 queued、dispatching、streaming、completed、failed、canceled 和 uncertain。详情投影 attempt 的发送、首字节、完成、HTTP 状态与稳定错误类别，不返回请求正文、响应正文或 secret；成员详情不返回 Provider 和上游凭据标签。未知 Token 和成本保持未知，不由页面估算。

总览和运维监控把 operations owner 返回的 24 小时请求、Token、终态、错误与上游 API Key 状态投影为趋势和构成图；订阅与额度只把当前服务端分页中的已用/剩余 Token 投影成堆叠条；上游成本只按当前筛选页和币种分别投影模型成本。图表不跨页、跨币种或跨 owner 补算未知数据，空结果保持空，精确值继续由指标和表格承载。

Runtime metrics 唯一拥有 admission、租约、Provider attempt、quota、请求恢复和后台 Responses 指标；Caddy 对公网拒绝 `/metrics`，Prometheus 从 backend 网络逐实例抓取。`console-concept/` 只保留为可独立打开的信息架构参考，不接真实 API、不进入生产构建，也不拥有第二套业务事实。
