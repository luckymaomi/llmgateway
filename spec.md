...
- 套餐拥有服务端自动生成的稳定标识，以及管理员维护的自然名称、说明、类型、状态和当前发布版本；内部标识不作为控制台输入。当前类型为 Token Plan 与 Coding Plan，二者都由明确 Token 额度、有效期、RPM、TPM、并发和模型路由表达，不把规则散落到 Provider adapter。
