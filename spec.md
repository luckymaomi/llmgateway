...
`spec.md` 是 BirdAPI 产品、架构、边界与设计方向的唯一事实主干。

## 产品定位

BirdAPI 是面向多 Provider 的 LLM 聚合网关。它对外提供统一 API，并集中管理上游账号、模型、密钥、额度、限流、调度、故障切换和用量记录，优先接入合法可用的免费模型、免费套餐与免费额度。

“免费”指 BirdAPI 优先收集和接入用户可合法使用的免费模型、免费套餐或免费额度，不代表任何上游永久免费、无限额度或稳定不变。

## 核心体验

1. 管理员接入多个 Provider 及其独立上游凭据，配置模型、分组、优先级和限制。
2. BirdAPI 为调用方签发统一的网关 API Key。
3. 调用方使用一个 Base URL 和一个 BirdAPI Key 访问已授权模型。
...
- Control Plane 管理 Provider、账号、模型、分组、BirdAPI Key、策略、用量、日志和健康状态，并向请求数据面发布已生效配置。
...
- BirdAPI 管理网关调用方与上游账号，但不提供批量注册、接码、养号、绕过风控、规避平台条款或突破免费额度的能力。
...
- 上游凭据、BirdAPI Key、账户状态和请求内容按敏感数据处理。
...
BirdAPI 采用 MIT License，作为公开 GitHub 项目协作和分发。
