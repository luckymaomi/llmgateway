---
name: plan
description: 在 BirdAPI 正式实现阶段编写和执行单文件 plan.md。适用于中大型开发、跨模块修改、产品边界调整、架构硬化、生产级验收和任何不能靠一次小补丁完成的任务；单纯初始化或讨论阶段不创建 plan.md。
---
...
1. 读取根目录 `AGENTS.md`。
2. 读取 `.agents/skills/birdapi-dev/SKILL.md`。
3. 读取 `docs/architecture.md`，调查当前仓库、工作区、实现、测试、文档和运行事实。
