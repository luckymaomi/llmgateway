...
- 上游 API Key 只由管理员集中保管；成员拿到的是权限和额度受控的 LLMGateway Key。
...
脚本不会读取本地 Provider Key，也不会自动调用任何收费模型。
...
2. 进入“Provider 接入”，从 Agnes、智谱 GLM、Google Gemini 或硅基流动模板开始；模板会安装已核验端点和文本模型，自定义 Provider 仍可单独添加。
3. 进入“Provider API Key”，添加真实 Provider Key、绑定模型并执行“测试连接”。Provider Key 只在这里由管理员维护。
4. 回到“Provider 接入 / 发布”，依次执行“捕获当前配置”、校验和发布。未发布的模型不会进入数据面。
5. 进入“用量与额度 / 上游成本”新增模型价格，再到“成员额度”给实际调用用户分配额度。缺少生效价格或额度时，请求会在发送给 Provider 前被拒绝。
6. 进入“成员与 API Key / API Key”创建调用 Key，选择所属成员和授权模型。完整 Key 只显示一次；结果页会同时给出当前 Base URL，并可一次复制调用配置。
7. 在“成员与 API Key”中对该 Key 执行“测试请求”；成功后再接入自己的 SDK。

首位管理员是自助初始化，后续成员不是自由注册：管理员在“成员与 API Key / 邀请”创建一次性邀请码，成员从登录页进入“邀请注册”，提交后还要由管理员在成员列表审核通过。

登录后可从桌面或移动侧栏进入“更换密码”。操作需要当前密码；成功后当前会话继续有效，其他登录会话立即撤销。初始密码、当前密码和新密码都不会写入日志、审计或浏览器持久存储。

### 4. 让 SDK 调用

本地 Base URL 是 `http://127.0.0.1:8080/v1`，API Key 使用上一步生成的 LLMGateway Key，不是 Provider Key。以 Python OpenAI SDK 为例：
...
client = OpenAI(
    ...
    api_key="这里换成只显示一次的 LLMGateway Key",
)
...
Gateway Key 与 Provider Key 是两件事：前者给成员或应用调用 LLMGateway，后者让 LLMGateway 调用上游。

1. 在“我的 API Key”或管理员的“成员与 API Key / API Key”中点击旧 Key 的“更换”。
2. 复制只显示一次的新 Key，并先更新、验证所有客户端。
3. 此时旧 Key 仍可用，避免切换过程中突然中断。
4. 确认客户端全部切换后，再对旧 Key 点击“撤销”；撤销后使用它的请求会立即失败。

Provider Key 的更换在“Provider API Key”完成。先增加并探测新 Key、调整模型绑定并发布配置，确认流量接管后再停用旧 Key；免费资源域不会自动落到付费凭据。
...
| `python .\start_test.py everything` | 依次运行 full、provider、capacity、release，首个失败停止 | 约 30～60 分钟 |
