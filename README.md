...
Windows 开发入口允许出站请求解析到 `198.18.0.0/15`，用于兼容透明代理使用的 Fake-IP。这个放宽只属于本机开发配置；正式部署不继承，生产仍按显式 SSRF 网络策略失败关闭。代码内置 Provider catalog 只提供经过校验的平台、端点和模型元数据；创建资源池不发送上游请求，添加上游 API Key 后的探测和正式请求仍会执行完整地址校验。
...
4. 在“上游 API Key”添加真实凭据、绑定池内模型并执行探测。priority、weight、RPM、TPM 和并发按上游合同设置，限额 `0` 表示不额外限制。合法写入提交后直接进入实时资格读取，不需要配置发布。
...
停止命令保留数据库卷，下次启动会接着使用原成员、资源池、套餐、订阅、Key 和账本。
